...
class Versa_AI_Writer {
    ...
    public function run(): void {
        ...
        $messages      = [
            [
                'role'    => 'system',
                'content' => 'You are an SEO content writer. Reply with strict JSON only.',
            ],
            [
                'role'    => 'user',
                'content' => $prompt,
    ...
    private function fetch_next_queue_item(): ?array {
        ...
        $row   = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$table}
                 WHERE status = %s
                 AND (scheduled_for_date IS NULL OR scheduled_for_date <= %s)
                 ORDER BY scheduled_for_date ASC, created_at ASC
                 LIMIT 1",
                'queued',
                $today
            ),
    ...
    private function build_prompt( array $profile, array $queue_item, array $recent_titles ): string {
        ...
        $image_count    = isset( $profile['writer_image_count'] )
            ? (int) $profile['writer_image_count']
            : 0;
        ...
        return Versa_AI_Prompts::render(
            ...
            function () use (
                ...
                $lines[] = 'Structure: one H1 matching the title; clear intro; 3-5 body sections with H2/H3; CTA; FAQ section with 3-5 Q&A.';
                $lines[] = 'Depth: satisfy search intent; aim ~900-' . (int) $profile['max_words_per_post'] . ' words.';
                $lines[] = 'Style: second-person POV; active voice; short sentences; scannable sections.';
                $lines[] = 'Links: 2-5 natural internal links only; never invent URLs; no external links.';
                $lines[] = 'HTML only, no Markdown. Clean tags only.';
                $lines[] = 'FAQ: concise factual answers.';

                if ( $recent_titles ) {
                    $lines[] = 'Avoid duplicating these recent posts: ' . implode( '; ', $recent_titles );
                }

                if ( $include_images && $image_count > 0 ) {
                    $lines[] = 'Insert ' . $image_count . ' images using <figure><img><figcaption>. Must include alt text.';
                    $lines[] = 'Use lightweight external URLs (Unsplash). No data URIs.';
                }

                $lines[] = 'Compliance: no prices, legal claims, guarantees.';
                $lines[] = 'Return strict JSON with keys: content_html, seo_title, seo_description, faq_schema_json.';
    ...
    private function decode_writer_payload( string $content ): ?array {
        ...
        return [
            ...
            'faq_schema_json' => $decoded['faq_schema_json'] ?? null,
        ];
    }
    ...
    private function create_post_from_payload( array $queue_item, array $payload, array $profile ): ?int {
        ...
        if ( ! empty( $payload['seo_title'] ) ) {
            update_post_meta(
                $post_id,
                'rank_math_title',
                wp_strip_all_tags( $payload['seo_title'] )
            );
        }
        if ( ! empty( $payload['seo_description'] ) ) {
            update_post_meta(
                ...
                wp_strip_all_tags( $payload['seo_description'] )
            );
        }
        ...
        if ( ! empty( $payload['faq_schema_json'] ) ) {
            update_post_meta(
                $post_id,
                'versa_ai_faq_schema',
                wp_json_encode( $payload['faq_schema_json'] )
            );
        }
